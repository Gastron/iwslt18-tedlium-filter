...
if [ $stage -le 1 ]; then
  ...
  #Aalto custom for IWSLT:
  iwslt18-tedlium-filter/filter_segments.sh 
  ...
  # Also note .orig.filt here
  for dset in dev test train; do
    utils/data/modify_speaker_info.sh --seconds-per-spk-max 180 data/${dset}.orig.filt data/${dset}
  done
fi
